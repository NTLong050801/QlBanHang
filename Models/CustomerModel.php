...
class CustomerModel extends BaseModel
{
    ...
    public function total_page_search_sp($val){
        $sql = "SELECT count(IDSanPham) as SLSP from sanpham where TenSP like '%$val%'";
        $query = $this->query($sql);
        $tongsp = mysqli_fetch_assoc($query)['SLSP'];
        $tongsotrang = ceil($tongsp/4);
        return $tongsotrang;
    }
}
